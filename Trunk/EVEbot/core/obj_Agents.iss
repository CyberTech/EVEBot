...
objectdef obj_Agents
{
	...
	function RequestMission()
	{
		...
		left:Set[${details.Find["<img src=\"factionlogo:"]}]
		if ${left} > 0
		{
			UI:UpdateConsole["obj_Agents: DEBUG: Found \"factionlogo\" at ${left}."]
			left:Inc[22]
			right:Set[${details.Mid[${left},16].Find["\" "]}]
			if ${right} > ${left}
			{
				UI:UpdateConsole["obj_Agents: DEBUG: left = ${left}"]	
				UI:UpdateConsole["obj_Agents: DEBUG: right = ${right}"]	
				UI:UpdateConsole["obj_Agents: DEBUG: string = ${details.Mid[${left},${Math.Calc[${right}-${left}]}]}"]	
				factionID:Set[${details.Mid[${left},${Math.Calc[${right}-${left}]}]}]	
		...
		left:Set[${details.Find["<img src=\"typeicon:"]}]
		if ${left} > 0
		{
			...
			if ${right} > ${left} 
			{
				...
				UI:UpdateConsole["obj_Agents: DEBUG: right = ${right}"]	
				UI:UpdateConsole["obj_Agents: DEBUG: string = ${details.Mid[${left},${Math.Calc[${right}-${left}]}]}"]	
				typeID:Set[${details.Mid[${left},${Math.Calc[${right}-${left}]}]}]	
		...
		right:Set[${details.Find["msup3"]}]
		if ${right} > 0
		{
			UI:UpdateConsole["obj_Agents: DEBUG: Found \"msup3\" at ${right}."]
			right:Dec
			left:Set[${details.Mid[${Math.Calc[${right}-16]},16].Find[" ("]}]
			if ${right} > ${left}
			{
				UI:UpdateConsole["obj_Agents: DEBUG: left = ${left}"]	
				UI:UpdateConsole["obj_Agents: DEBUG: right = ${right}"]	
				UI:UpdateConsole["obj_Agents: DEBUG: string = ${details.Mid[${left},${Math.Calc[${right}-${left}]}]}"]	
				volume:Set[${details.Mid[${left},${Math.Calc[${right}-${left}]}]}]	
