...
objectdef obj_Miner
{
	...
	function Mine()
	{
		...
		; Changed for SoloJetcanMiner
		while ( !${EVEBot.ReturnToStation} && \
						!${Ship.CargoFull} && \
						!( ${Config.Common.BotModeName.Equal[SoloJetcanMiner]} && ${Ship.CargoHalfFull}) )
		{
			...
			; Changed for SoloJetcanMiner
			if ${Config.Miner.UseJetCan} && ${Ship.CargoHalfFull} && !${Config.Common.BotModeName.Equal[SoloJetcanMiner]}
			{
