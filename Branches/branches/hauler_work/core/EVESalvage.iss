...
function main(... Args)
{
  ...
  ; 'Args' is an array ... arrays are static.  Copying to an index just in case we have a desire at some point to add/remove elements.
	if ${Args.Size} > 0
	{
		do
		{
			...
			elseif (${Args[${Iterator}].Equal[-LOOT]} || ${Args[${Iterator}].Equal[-loot]})
				DoLoot:Set[TRUE]
			else
  ...
  if (${SalvageLocationLabels.Used} <= 0)
  {
  	echo "- Syntax:  'run EVESalvage -loot <bookmarklabel1> <bookmarklabel2> ...'"
  	echo "-          'run EVESalvage <bookmarklabel1> <bookmarklabel2> ...'"
  	return
  }  
