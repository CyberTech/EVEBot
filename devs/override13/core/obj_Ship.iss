...
objectdef obj_Ship
{
	...
	method UpdateModuleList()
	{
		...
		{
			call UpdateHudStatus "DEBUG: obj_Ship:UpdateModuleList - No modules found"
			return
		}

		variable iterator Module

		call UpdateHudStatus "Module Inventory:"
		...
		call UpdateHudStatus "Passive Modules:"
		This.ModuleList_Passive:GetIterator[Module]
		if ${Module:First(exists)}
		do
		{
			call UpdateHudStatus "    Slot: ${Module.Value.ToItem.Slot}  ${Module.Value.ToItem.Name}"
		}
		while ${Module:Next(exists)}

		call UpdateHudStatus "Mining Modules:"
	...
	member:string LoadedMiningLaserCrystal(string SlotName)
	{
		...
		do
		{
			...
			{
				call UpdateHudStatus "obj_Ship:LoadedMiningLaserCrystal Returning ${Module.Value.Charge.Name.Token[1, " "]}
	...
	function ChangeMiningLaserCrystal(string OreType, string SlotName)
	{
		...
		{
			call UpdateHudStatus "DEBUG: Current crystal in ${SlotName} is ${LoadedAmmo}, looking for ${OreType}"
			...
			do
			{
				...
				call UpdateHudStatus "Testing ${OreType} contains ${CrystalType}"
	...
	method ValidateModuleTargets()
	{
		...
		do
		{
			...
			{
				call UpdateHudStatus "${Module.Value.Name} has non-existent target, deactivating"
				Module.Value:Click
			}
		}
		while ${Module:Next(exists)}
	...
	function Approach(int EntityID)
	{
		if ${Entity[${EntityID}](exists)}
		{
			...
			{
				call UpdateHudStatus "DEBUG: obj_Ship:Approach: ${Entity[${EntityID}].Name} is getting further away!  Is it moving? Are we stuck, or colliding?"
			}
			
			if ${Entity[${EntityID}](exists)} && \
				${OriginalDistance} == ${Entity[${EntityID}].Distance}
			{
				call UpdateHudStatus "DEBUG: obj_Ship:Approach: We may be stuck or colliding"
			}
		}
	}			
	...
	{ 
		if (${Id} <= 0)
		{
			call UpdateHudStatus "Error: obj_Ship:WarpToID: Id is <= 0 (${Id})"
			return
		}
		
		if !${Entity[${Id}](exists)}
		{
			call UpdateHudStatus "Error: obj_Ship:WarpToID: No entity matched the ID given."
			return
		}
