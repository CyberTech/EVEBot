/*
	...
	Methods and Members for handling pulse timing and randomization.

	Could also be used as a base class, but then we'd end up with some classes having 2 or 3 bases.
...
objectdef obj_PulseTimer
{
	...
	method SetIntervals(float MinInterval, float MaxInterval)
	{
		...
		UI:UpdateConsole["${LogPrefix}: Initialized Min: ${MinInterval} Max: ${MaxInterval}", LOG_DEBUG]
	}
