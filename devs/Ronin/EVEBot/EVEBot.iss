...
function SetBotState()
{
	...
	if ${Me.Ship.UsedCargoCapacity} <= ${Math.Calc[${Me.Ship.CargoCapacity}-4]}
	{
	 	botstate:Set["MINE"]
		return
	}
	...
	while ${play}
	{
		...
		switch ${botstate}
		{
			case BASE
				...
				while ${Math.Calc[${Me.Ship.UsedCargoCapacity}>(${Me.Ship.CargoCapacity}*0.20)]}
				{
					call TransferToHangar
					wait 10
				}				
			...
			case CARGOFULL
				...
				call UpdateHudStatus "Setting main station ${Entity[CategoryID,3].Name} with ID ${stationloc}"
				call UpdateHudStatus "My ship is full"
				call Dock ${stationloc}
				wait 40
				break
		}
